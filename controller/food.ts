...
router.post("/cook", async (req, res) => {
    ...
    try {
        ...
        const [recipeIngredients]: any = await connection.query(
            "SELECT ing_id, amount FROM food_component WHERE food_id = ?", [food_id]
        );

        // 2. ตรวจสอบคลังและคำนวณการตัดสต็อก
        for (let item of recipeIngredients) {
            const [userIng]: any = await connection.query(
                "SELECT amount FROM user_ingredient WHERE u_id = ? AND ing_id = ?", [uid, item.ing_id]
            );
            ...
            if (newAmount === 0) {
                await connection.query("DELETE FROM user_ingredient WHERE u_id = ? AND ing_id = ?", [uid, item.ing_id]);
            } else {
                await connection.query("UPDATE user_ingredient SET amount = ? WHERE u_id = ? AND ing_id = ?", [newAmount, uid, item.ing_id]);
            }
        }

        await connection.commit();
        res.status(200).json({ message: "ปรุงอาหารสำเร็จ! วัตถุดิบถูกตัดเรียบร้อย" });
    } catch (err: any) {
